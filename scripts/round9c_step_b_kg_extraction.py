...
ROLLBACK = OUT / "round9c_kg_rollback.cypher"
...
def write_fact(session, case: dict, fact: dict) -> dict:
    ...
    obs_id = f"{c.ROUND9C_BATCH}__{case['case_id']}__{metric}__{year}__{fact.get('fact_id', '')}"
...
SET obs.value = $value,
    ...
    obs.kg_batch = $batch,
    obs.batch_id = $batch,
    obs.extraction_method = 'round9c_contract_targeted',
...
""",
        ...
        quote=quote,
        batch=c.ROUND9C_BATCH,
        source_fact_id=fact.get("fact_id", ""),
        case_id=case["case_id"],
        source_dataset=case["source_dataset"],
    )
    return {"case_id": case["case_id"], "ticker": ticker, "obs_id": obs_id, "metric": metric, "year": year, "value": value, "batch_id": c.ROUND9C_BATCH}
...
def main() -> None:
    ...
    for path in [TRACE, WRITE_LOG, FAILED]:
        if path.exists():
            path.unlink()
    ROLLBACK.write_text(f"MATCH (obs:LLMObservation {{kg_batch: '{c.ROUND9C_BATCH}'}}) DETACH DELETE obs;\n", encoding="utf-8", newline="\n")
    cases = {row["case_id"]: row for row in c.load_all_round9c_cases()}
    ...
    try:
        ...
        with driver.session(database=env["NEO4J_DATABASE"]) as session:
            existing = session.run("MATCH (obs:LLMObservation {kg_batch: $batch}) RETURN count(obs) AS n", batch=c.ROUND9C_BATCH).single()["n"]
            if int(existing):
                print(json.dumps({"batch_id": c.ROUND9C_BATCH, "existing": existing, "action": "append_or_update"}, ensure_ascii=False))
    ...
    if success < 0.8:
        raise RuntimeError(f"Round9C KG write success below threshold: {success}")
